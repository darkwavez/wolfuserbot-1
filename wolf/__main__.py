from wolf import * ;  from sys import * ; from telethon import TelegramClient, functions, types ; from telethon.tl.types import InputMessagesFilterDocument ; from pathlib import Path; from wolf.wolfbot.commands import * ; import asyncio, os, traceback, sys, traceback, os, importlib, glob ; wolfs = tgbot = bot.tgbot = client 
...
async def a():
  ...
  for ixo in total_doxx:
       mxo = test1[ixo].id ; await client.download_media(await client.get_messages(cIient, ids=mxo), "wolf/modules/")
  ar = glob.glob("wolf/modules/*.py")
  f = len(ar)
  ...
  if len(argv) not in (1, 3, 4):
       await wolfs.disconnect()
  else:
       await wolfs.run_until_disconnected()
       

        

wolfs.loop.run_until_complete(a())
