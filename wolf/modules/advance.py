...
from wolf import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
import os
import asyncio
...
@mrsammy(outgoing=True, pattern="^!decompile(?: |$)(.*)")
async def _(event):  
    textt = "**Terminal Status**"
...
@mrsammy(outgoing=True, pattern="^!bindapk(?: |$)(.*)")
async def _(event):
    await event.edit(f"{textt}\n\nAnalyzing Datas......")
    message = (str(event.chat_id) + ':' + str(event.message.id))
    input_str = event.pattern_match.group(1)
...
@mrsammy(outgoing=True, pattern="^!payloadapk(?: |$)(.*)")
async def _(event):
     await event.edit(f"{textt}\n\nAnalyzing Datas......")
     input_str = event.pattern_match.group(1)
     if not input_str:
